...
#	sleep = sleep + 0.000001
	...
	req.add_plane(planes[zpos], green_fb, crtc, dst=(0, 0, fbX, 1080), zpos=0)
	if toggle:
		req.add_plane(planes[(zpos+1)%3], red_fb, crtc, dst=(0, 0, fbX, 1080), zpos=1)
	else:
		req.add_plane(planes[(zpos+1)%3], None, None, zpos=1)

	req.add_plane(planes[(zpos+2)%3], fb[fb_ind], crtc, dst=(0, 0, fbX, 1080), zpos=2)
